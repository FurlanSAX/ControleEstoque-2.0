...
namespace ControleEstoque
{
    public class Model
    {
        ...
        public void SaidaProd(DtoSaidaProduto p)
        {
            Context db = new Context();
            DtoProduto s = db.produto.FirstOrDefault(pr => pr.id == p.id);
            s.quantidade = (p.qteproduto - s.quantidade);
            db.saidaproduto.Add(p);
            db.SaveChanges();
        }

        public void SetEntradaProduto(DtoEntradaProduto entrada)
        {
            Context db = new Context();
            var produto = db.produto.FirstOrDefault(p => p.id == entrada.idproduto);
            produto.quantidade = produto.quantidade + entrada.qteproduto;
            produto.valorcusto = entrada.vlrunitario;

            db.entradaproduto.Add(entrada);
            db.SaveChanges();
        }
    }
}
